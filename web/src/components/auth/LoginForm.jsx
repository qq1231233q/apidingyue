...
const LoginForm = () => {
  ...
  const handlePasskeyLogin = async () => {
    ...
    if (!passkeySupported) {
      showInfo(t('当前环境无法使用通行密钥（Passkey）登录'));
      return;
    }
    if (!window.PublicKeyCredential) {
      showInfo(t('当前浏览器不支持通行密钥（Passkey）'));
      return;
    }
    ...
    try {
      ...
      if (!success) {
        showError(message || t('无法发起通行密钥（Passkey）登录'));
        return;
      }
      ...
      if (!payload) {
        showError(t('通行密钥（Passkey）验证失败，请重试'));
        return;
      }
      ...
      if (finish.success) {
        ...
        showError(finish.message || t('通行密钥（Passkey）登录失败，请重试'));
      }
    } catch (error) {
      if (error?.name === 'AbortError') {
        showInfo(t('已取消通行密钥（Passkey）登录'));
      } else {
        showError(t('通行密钥（Passkey）登录失败，请重试'));
      }
    } finally {
            ...
            <div className='px-2 py-8'>
              <div className='space-y-3'>
                ...
                {status.passkey_login && passkeySupported && (
                  ...
                  >
                    <span className='ml-3'>{t('使用通行密钥（Passkey）登录')}</span>
                  </Button>
                )}
                ...
                >
                  <span className='ml-3'>{t('使用邮箱或用户名登录')}</span>
                </Button>
              </div>
  ...
  return (
    <div className='page-tech-shell relative min-h-screen overflow-hidden flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8'>
      {/* 背景模糊晕染球 */}
      <div
